/* =========================================================
   후한의원 강남점 어드민 — GitHub에 직접 저장 (어디서든 발행)
   ※ 전주점(jjwart)이 아니라 강남점(gnwart) 저장소로만 올라갑니다.
...
const GH_REPO   = 'gnwart';
const GH_BRANCH = 'main';
const GH_TOKEN_KEY = 'hoo-gangnam-admin-gh-token';
...
/* ---------- 옛 자리표시 사진 경로 자동 정리 ----------
   초기 컬럼들의 대표 이미지가 doctor-hong.svg 로 저장돼 있는데
   실제 사진은 .jpg 로 교체되어 사진이 깨져 보였다.
   저장소에 .svg 사진은 하나도 없으므로, 남아있는 경로만 .jpg 로 바꿔준다. */
(function fixSvgImagePaths(){
  let changed = false;
  for (const col of state.columns) {
    if (col.image && /^assets\/images\/.+\.svg$/i.test(col.image)) {
      col.image = col.image.replace(/\.svg$/i, '.jpg');
      changed = true;
    }
  }
